...
export function createSession(token: string, session: Omit<Session, 'createdAt'>): void {
  sessions.set(token, { ...session, createdAt: new Date() });
  logger.info('Session created', { token, smartflowCallId: session.smartflowCallId, retellCallId: session.retellCallId });
}
...
export function deleteSession(token: string): void {
  ...
  if (session) {
    ...
    logger.info('Session deleted', { token, smartflowCallId: session.smartflowCallId });
  }
}
